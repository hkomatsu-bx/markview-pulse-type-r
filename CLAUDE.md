...
- `src/core/` — pure, framework-free logic:
  ...
  - `markdown.ts` — markdown-it config. `html: true` allows raw HTML, then **DOMPurify sanitizes** the rendered output (GitHub's "permit → sanitize" model; untrusted `.md` is assumed). `renderMarkdown` is the single XSS chokepoint feeding every `innerHTML` sink; sanitization drops `<script>`, event-handler attributes, `javascript:`/`target`, and `<style>`/`style=`. Single source of renderer config. Prepends the YAML front-matter table (`frontMatter.ts`) and emits ` ```mermaid ` fences as `<pre class="mermaid">` for the UI layer to render. Wires highlight.js (core + the languages registered in this file; unregistered languages fall back to plain escaping).
