...
本ファイルは Markview Pulse Type R が利用する第三者ソフトウェアのライセンス**一覧**（SPDX 識別子）である。
各ライセンスの全文は含まない。完全な条文は各パッケージ配布物に同梱されている。

- 本体ライセンス: MIT（[LICENSE](LICENSE) を参照、Copyright (c) 2026 Markview Pulse Type R contributors）
- 生成方法: Rust = `cargo metadata`（resolve グラフを normal 依存のみ辿って抽出。dev/build 依存はバイナリに同梱されないため除外）。npm = `license-checker --production`。
- 生成ツールはグローバル導入していない。条文同梱が必要な対外配布時は cargo-about 等での再生成を推奨。

## Rust crates（同梱バイナリにリンクされる normal 依存: 454 件）

ライセンス内訳: MIT OR Apache-2.0 (207), MIT (107), Apache-2.0 OR MIT (45), MIT/Apache-2.0 (18), Unicode-3.0 (18), Zlib OR Apache-2.0 OR MIT (17), Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT (5), MPL-2.0 (5), Unlicense OR MIT (4), Apache-2.0/MIT (3), ISC (3), Apache-2.0 (2), BSD-3-Clause (2), BSD-3-Clause OR MIT OR Apache-2.0 (2), MIT OR Apache-2.0 OR LGPL-2.1-or-later (2), MIT OR Apache-2.0 OR Zlib (2), Unlicense/MIT (2), (MIT OR Apache-2.0) AND Unicode-3.0 (1), 0BSD OR MIT OR Apache-2.0 (1), Apache-2.0 / MIT (1), Apache-2.0 AND MIT (1), BSD-3-Clause AND MIT (1), BSD-3-Clause/MIT (1), CC0-1.0 (1), CC0-1.0 OR MIT-0 OR Apache-2.0 (1), MIT OR Zlib OR Apache-2.0 (1), Zlib (1)
...
## npm packages（製品同梱の production 依存: 12 件）

ライセンス内訳: MIT (6), MIT OR Apache-2.0 (2), Apache-2.0 OR MIT (1), BSD-2-Clause (1), BSD-3-Clause (1), Python-2.0 (1)
