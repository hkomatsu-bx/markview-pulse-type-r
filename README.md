...
| PDF 書き出し | 「PDF で保存」で表示内容を PDF 化。テキストは選択・検索可能（WebView2 の PrintToPdf を使用）。各ページにファイル名とページ番号が入り、画面がダークでも常にライト配色で出力する |
| 印刷 | 「印刷」または **Ctrl+P** でプリンタへ印刷。ツールバー・タブ・ステータスバーは除外される |
| コマンドメニュー | ツールバー右端の「…」に副次コマンド（ファイルを開く・印刷・PDF で保存・エディタで開く・本文幅・テーマ）を集約 |
...
- **PDF で保存**: 「…」メニューまたは右クリックメニューから選び、保存先を指定する。テキストは選択・検索できる（画像化されない）。各ページの上部にファイル名、下部にページ番号が入る。画面のテーマに依らず常にライト配色で出力する。
- **印刷**: 「印刷」または **Ctrl+P** でプリンタの印刷ダイアログを開く。PDF が目的の場合は「PDF で保存」を使う（印刷ダイアログ経由の PDF 化は、プリンタドライバによって文字がアウトライン化され選択できなくなる）。

### コマンドメニュー（「…」）

- ツールバー右端の「…」をクリックすると、**ファイルを開く／印刷／PDF で保存／エディタで開く／本文幅／テーマ** が開く。
